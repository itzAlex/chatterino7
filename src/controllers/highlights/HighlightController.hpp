...
struct HighlightCheck {
    using Checker = std::function<boost::optional<HighlightResult>(
        ...
        const QString &senderName, const QString &originalMessage, bool self, const QString &channel)>;
    Checker cb;
};
...
public:
     ...
     * Alex: Added channel argument to check if the highlight should appear there
     **/
    [[nodiscard]] std::pair<bool, HighlightResult> check(
        const MessageParseArgs &args, const std::vector<Badge> &badges,
        const QString &senderName, const QString &originalMessage, const QString &channel = "") const;
